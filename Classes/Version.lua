...
function Version:notBackwardsCompatibleNotice()
    ...
    if (serverTime - self.lastNotBackwardsCompatibleNotice >= 30) then
        ...
        GL:error(L.LANG_GARGUL_VERSION_OUTOFDATE);
    end
end
...
function Version:notifyOfLatestVersion()
    ...
    local notify = function ()
        GL:error(L.LANG_GARGUL_VERSION_OUTDATE_ERROR);
        GL:warning((L.LANG_GARGUL_V_U_WARING):format(self.latest));
        ...
        GL.Interface.Alerts:fire("GargulNotification", {
            message = string.format(L.LANG_GARGUL_NOTE),
        });
    end
...
function Version:inspectGroup()
    ...
    if (not GL.User.isInGroup) then
        return GL:error(L.LANG_NOTINGROUP);
    end

    GL:message(L.LANG_V_CHECK_M);
...
function Version:finishInspectGroup(CommMessage)
    ...
    if (self.isOutOfDate) then
        GL:error(string.format(L.LANG_V_OWN, self.current, self.latest));
    else
        GL:success(string.format(L.LANG_V_OWN_UTD, self.current));
    end

    -- List all player that are up-to-date
    if (#upToDate >= 1) then
        GL:message(L.LANG_V_ALL_PLAYER);
