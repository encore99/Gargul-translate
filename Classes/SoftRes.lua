...
function SoftRes:_init()
    ...
    GL.Ace:ScheduleTimer(function()
        ...
        if (success and result) then
            ...
            GL:error(L.LANG_FAILED_TO_CONNECT_LOOTRESERVE);
    ...
    GL.Events:register("AlertsSoftresImported", "GL.SOFTRES_IMPORTED", function ()
        if (not GL.Alerts:fire("SoftRes", "Import successful!")) then
            GL:success(L.LANG_IMPORT_SUCCESSFUL);
        end
    end);
...
function SoftRes:handleWhisperCommand(_, message, sender)
    ...
    GL:onItemLoadDo(ItemIDs, function (Details)
        ...
        if (not GL:empty(Entries)) then
            GL:sendChatMessage(
                L.LANG_SOFTRESERV_RESERVED .. table.concat(Entries, ", "),
                "WHISPER", nil, sender
            );
...
function SoftRes:tooltipLines(itemLink)
    ...
    if (hardReservedFor or hardReservedNote) then
        tinsert(Lines, string.format("\n|cFFcc2743%s|r", L.LANG_SOFTRESERVE_HARDRESERVE));
        if (hardReservedFor) then
            local forString = string.format(L.LANG_SOFTRESERVER_FOR,
        ...
        if (hardReservedNote) then
            local noteString = string.format(L.LANG_SOFTRESERVER_NOTE,
                hardReservedNote
            );
    ...
    -- Add the header
    tinsert(Lines, string.format("\n|cFFEFB8CD%s|r", L.LANG_SOFTRESERVED_BY));
...
function SoftRes:import(data, openOverview)
    ...
    if (GL:empty(data)) then
        GL.Interface:get("SoftRes.Importer", "Label.StatusMessage"):SetText(L.LANG_SOFTRESERVE_INVALID_DATA);
        return false;
    end
    ...
    if (reportStatus) then
        ...
        if (not GL:empty(PlayersWhoDidntReserve)) then
            ...
            GL:warning(L.LANG_SOFTRESERVE_NO_RESERVE_PLAYER);
    ...
    if (not GL.User.isInGroup
        ...
        if (GL.Settings:get("SoftRes.enableWhisperCommand", true)
            ...
            GL:sendChatMessage(
                string.format(L.LANG_SOFTRESERVE_IMPORT_GARGUL),
                "GROUP"
            );
        end
    end
...
function SoftRes:importGargulData(data)
    ...
    if (not base64DecodeSucceeded) then
        local errorMessage = L.LANG_SOFTRESERVE_UNABLE_TO_BASE_64;
    ...
    if (not zlibDecodeSucceeded) then
        local errorMessage = L.LANG_SOFTRESERVE_UNABLE_TO_ZLIB_DECODE_DATA;
        GL.Interface:get("SoftRes.Importer", "Label.StatusMessage"):SetText(errorMessage);
    ...
    if (not jsonDecodeSucceeded) then
        local errorMessage = L.LANG_SOFTRESERVER_UNABLE_TO_DECODE_DATA;
        GL.Interface:get("SoftRes.Importer", "Label.StatusMessage"):SetText(errorMessage);
    ...
    local function throwGenericInvalidDataError()
        local errorMessage = L.LANG_SOFTRESERVE_INVALID_DATA_PROVIDED;
        GL.Interface:get("SoftRes.Importer", "Label.StatusMessage"):SetText(errorMessage);
    ...
    if (differentPlusOnes) then
        ...
        GL.Interface.Dialogs.PopupDialog:open{
            question = L.LANG_PLUSONE_PROVIDED,
            OnYes = function ()
...
function SoftRes:importCSVData(data, reportStatus)
    ...
    if (reportStatus) then
        GL:warning(L.LANG_SOFTRES_WA_DEPRECATED);
    end
...
function SoftRes:broadcast()
    ...
    if (not GL.User.isInGroup) then
        GL:warning(L.LANG_BROADCAST_NO_GROUP);
        return false;
    end
    ...
    if (not self:available()
        ...
        GL:warning(L.LANG_BROADCAST_SOFTRESERV_DATA_FIRST);
        return false;
    end
    ...
    GL.Ace:ScheduleTimer(function ()
        GL:success(L.LANG_BROADCAST_FINISHED);
...
function SoftRes:receiveSoftRes(CommMessage)
    ...
    if (not GL:empty(importString)) then
        GL:warning(L.LANG_SOFTRES_DATA_INCOMING_FROM .. CommMessage.Sender.name);
        return self:import(importString);
    end

    GL:warning(L.LANG_SOFTRES_DARA_RECEIVED_FROM .. CommMessage.Sender.name);
    return false;
end
...
function SoftRes:postLink()
    ...
    if (not softResLink) then
        GL:warning(L.LANG_SOFTRES_NO_URL);
        return false;
    end
...
function SoftRes:postMissingSoftReserves()
    ...
    if (not self:available()) then
        GL:warning(L.LANG_SOFTRES_DATA_AVAILABLE_IMPORT);
        return false;
    end
    ...
    if (#PlayerNames < 1) then
        GL:success(L.LANG_SOFTRES_FILLED_SOFTRES);
        return true;
    end

    -- Post the link in the chat for all group members to see
    GL:sendChatMessage(
        L.LANG_MISSING_SOFTRES .. table.concat(PlayerNames, ", "),
        "GROUP"
    );
...
function SoftRes:postDiscordLink()
    ...
    if (not discordLink) then
        GL:warning(L.LANG_SOFTRES_NO_DISCORD);
        return false;
    end
