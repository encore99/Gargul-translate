    ...
    (function()
        ...
        if (iTradedGold and not iTradedItems and not iEnchantedSomething) then
            ...
            if (theyDidNothing) then
                return GL:sendChatMessage(string.format(L.LANG_TRADE_GIVE, goldTradedByMe, Details.partner), channel, nil, recipient);
            end
            ...
            if (theyEnchantedSomething and not theyTradedItems) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY,
                    Details.partner,
            ...
            GL:sendChatMessage(string.format(L.LANG_TRADE_GIVE, goldTradedByMe, Details.partner), channel, nil, recipient);
        end
        ...
        if (theyTradedGold and not theyTradedItems and not theyEnchantedSomething) then
            ...
            if (iDidNothing) then
                return GL:sendChatMessage(string.format(L.LANG_RECEIVED_FROM, goldTradedByThem, Details.partner), channel, nil, recipient);
            end
            ...
            if (iEnchantedSomething and not iTradedItems) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_WITH_FOR_AND,
            ...
            GL:sendChatMessage(string.format(L.LANG_RECEIVED_FROM, goldTradedByThem, Details.partner), channel, nil, recipient);
        end
        ...
        if (iEnchantedSomething and not iTradedItems and theyDidNothing) then
            if (not iTradedGold) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_I_WITH_FOR,
                    EnchantedByMe.itemLink,
                    EnchantedByMe.enchantment,
                    Details.partner
                ), channel, nil, recipient);
            else
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_I_WITH_FOR_AND_GAVE,
                    EnchantedByMe.itemLink,
                    EnchantedByMe.enchantment,
                    Details.partner,
                    goldTradedByMe
        ...
        if (theyEnchantedSomething and not theyTradedItems and iDidNothing) then
            if (not theyTradedGold) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY_WITH,
                    Details.partner,
                    EnchantedByThem.itemLink,
                    EnchantedByThem.enchantment
                ), channel, nil, recipient);
            else
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY_WITH_AND_GAVE,
                    Details.partner,
                    EnchantedByThem.itemLink,
                    EnchantedByThem.enchantment,
                    goldTradedByThem
        ...
        if (iEnchantedSomething and not iTradedItems and theyEnchantedSomething and not theyTradedItems) then
            ...
            if (iTradedGold) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY_WITH_AND_ENTCHANT_THEIR_WITH_AND_GAVE,
                    Details.partner,
                    EnchantedByThem.itemLink,
                    EnchantedByThem.enchantment,
                    EnchantedByMe.itemLink,
            ...
            if (theyTradedGold) then
                return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY_WITH_AND_GAVE_ME_AND_ENCHANT_THEIR_WITH,
                    Details.partner,
                    EnchantedByThem.itemLink,
                    EnchantedByThem.enchantment,
                    goldTradedByThem,
            ...
            return GL:sendChatMessage(string.format(L.LANG_ENCHANT_MY_WITH_AND_ENCHANT_THEIR_WITH,
                Details.partner,
                EnchantedByThem.itemLink,
                EnchantedByThem.enchantment,
                EnchantedByMe.itemLink,
        ...
        if (iTradedItems) then
            ...
            if (iTradedGold) then
                message = string.format(L.LANG_TRADE_GOLD, goldTradedByMe);
                ...
                (function()
                    if (Entry.quantity <= 1) then
                        if (messageLength < 1) then
                            message = string.format(L.LANG_TRADE_GOLD, Entry.itemLink);
                            messageLength = messageLength + string.len(L.LANG_GAVE) + itemLinkLength;
                        ...
                        if (messageLength < 1) then
                            message = string.format(L.LANG_GAVE2, Entry.itemLink, Entry.quantity);
                            messageLength = messageLength + string.len(L.LANG_GAVE_X) + itemLinkLength + string.len(Entry.quantity);
            ...
            if (messageLength) then
                ...
                if (iEnchantedSomething) then
                    ...
                    newMessageLength = messageLength + string.len(L.LANG_TO_AND_ENCHANT_THEIR_WITH)
                    ...
                    if (newMessageLength >= 255 or itemsProcessed >= 5) then -- The enchant also includes an item link
                        ...
                        message = string.format(L.LANG_ENCHANT_TO_THEIR_WITH,
                            Details.partner,
                            EnchantedByMe.itemLink,
                            EnchantedByMe.enchantment
                        );
                        ...
                        message = string.format(L.LANG_TO_AND_ENCHANT_THEIR_WITH,
                            message,
                            Details.partner,
                            EnchantedByMe.itemLink,
                            EnchantedByMe.enchantment
                    ...
                    newMessageLength = messageLength + string.len(L.LANG_TO) + string.len(Details.partner);

                    if (newMessageLength >= 255) then
                        GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                        firstOutput = false;
                        message = string.format(L.LANG_TO1, Details.partner);
                        GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                    else
                        message = string.format(L.LANG_TO2, message, Details.partner);
                        GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                    end
                end
                ...
                if (iEnchantedSomething) then
                    ...
                    message = string.format(L.LANG_TO3, message, Details.partner);
                    GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                end
            end
        end
        ...
        if (theyTradedGold) then
            message = string.format(L.LANG_RECEIVED, goldTradedByThem);
            messageLength = string.len(message);
            itemsInMessage = 1;
        end
        ...
        for _, Entry in pairs(ItemsTradedByThem) do
            ...
            (function()
                if (Entry.quantity <= 1) then
                    if (messageLength < 1) then
                        message = string.format(L.LANG_RECEIVED, Entry.itemLink);
                        messageLength = messageLength + string.len(L.LANG_RECEIVED1) + itemLinkLength;
                    ...
                    if (messageLength < 1) then
                        message = string.format(L.LANG_RECEIVED2, Entry.itemLink, Entry.quantity);
                        messageLength = messageLength + string.len(L.LANG_RECEIVEDX) + itemLinkLength + string.len(Entry.quantity);
        ...
        if (messageLength) then
            ...
            if (theyEnchantedSomething) then
                ...
                newMessageLength = messageLength + string.len(L.LANG_FROM_AND_GOT_MY_ENCHANT)
                    + string.len(Details.partner)
                    + itemLinkLength
                ...
                if (newMessageLength >= 255 or itemsProcessed >= 5) then -- The enchant also includes an item link
                    ...
                    message = string.format(L.LANG_FROM_AND_GOT_MY_ENCHANT_WITH,
                        Details.partner,
                        EnchantedByThem.itemLink,
                        EnchantedByThem.enchantment
                    );
                    ...
                    message = string.format(L.LANG_FROM_AND_GOT_MY_ENCHANT_WITH1,
                        message,
                        Details.partner,
                        EnchantedByThem.itemLink,
                        EnchantedByThem.enchantment
                ...
                newMessageLength = messageLength + string.len(L.LANG_FROM) + string.len(Details.partner);

                if (newMessageLength >= 255) then
                    GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                    firstOutput = false;
                    message = string.format(L.LANG_FROM1, Details.partner);
                    GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                else
                    message = string.format(L.LANG_FROM2, message, Details.partner);
                    GL:sendChatMessage(message, channel, nil, recipient, firstOutput);
                end
            end
        else -- There's nothing left to announce. This happens VERY rarely
