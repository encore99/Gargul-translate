...
function RaidGroups:toCSV()
    ...
    if (not GL.User.isInGroup) then
        return GL:warning(L.LANG_NOTINRAID);
    end
...
function RaidGroups:drawImporter()
    ...
    Window:SetTitle(L.GARGUL_V .. GL.version);
    ...
    Explanation:SetText(L.LANG_EXPLANATION);
    ...
    Explanation:SetFullWidth(true);
    Explanation:SetText(L.LANG_GROUP_TEXT);
    Window:AddChild(Explanation);
    ...
    WikiButton:SetText(L.LANG_GROUP_WIKI);
    WikiButton:SetWidth(180);
    WikiButton:SetCallback("OnClick", function()
        GL.Interface.Dialogs.HyperlinkDialog:open{
            description = L.LANG_GROUP_DESCRIPTION,
    ...
    InviteButton:SetText(L.LANG_GROUP_BUTTON_INVITE);
    ...
    AttendanceCheckButton:SetText(L.LANG_GROUP_BUTTON_MISSING);
    ...
    SaveButton:SetText(L.LANG_GROUP_BUTTON_APPLY);
    ...
    SetTanksButton:SetText(L.LANG_GROUP_BUTTON_ASSIGN_TANKS);
...
function RaidGroups:invitePlayers(raidGroupCsv)
    for line in raidGroupCsv:gmatch("[^\n]+") do
        ...
        if (not group
            ...
            return GL:warning(L.LANG_GROUP_INVALID_RAID_GROUPS_PROVIDED);
        end
...
function RaidGroups:checkAttendance(raidGroupCsv, OutPutLabel)
    ...
    if (GL:empty(raidGroupCsv)) then
        return GL:warning(L.LANG_GROUP_INVALID_RAID_GROUPS_FORMAT_PROVIDED);
    end
    ...
    OutPutLabel:SetText(string.format(
        L.LANG_GROUP_PEOPLE_IN_RAID_SHOULDNT_BE,
        L.LANG_GROUP_PEOPLE_MISSING,
...
function RaidGroups:applyRaidGroups(raidGroupCsv)
    ...
    if (not GL.User.isInRaid) then
        return GL:warning(L.LANG_GROUP_NOTINRAID);
    end

    if (not GL.User.hasAssist) then
        return GL:warning(L.LANG_GROUP_NEED_LEAD);
    end

    if (self.migrationInProgress) then
        return GL:warning(L.LANG_GROUP_PROGRESS);
    end
        ...
        for _, playerName in pairs(Players) do
            ...
            if (group < 9 -- group 9 is a group we reserve for specifiying the tanks
                ...
                if (PlayersOnTheRoster[playerName]) then
                    return GL:warning(string.format(L.LANG_GROUP_WARNING, playerName));
                end
    ...
    for _, Raider in pairs(GL.User:groupMembers()) do
        ...
        if (UnitAffectingCombat("raid" .. Raider.index)) then
            return GL:warning(string.format(L.LANG_GROUP_SORT_IN_COMBAT, Raider.name));
        end
    ...
    if (UnwantedPlayers[1]) then
        ...
        return GL:warning(string.format(
            L.LANG_GROUP_PLAYERS_NOT_A_PART_OF_ROSER,
    ...
    if (migrationCount > 0) then
        GL:message(L.LANG_GROUP_APPLY_ROSTER);
...
function RaidGroups:updateTankAssignmentButton()
    ...
    if (not self.rosterString
        ...
        return GL:warning(L.LANG_GROUP_NO_ROSTER);
    end
    ...
    if (not Tanks) then
        return GL:message(L.LANG_GROUP_NO_TANKS_DEFINED);
    end
    ...
    if (macros and macros ~= "") then
        ...
        GL:success(L.LANG_GROUP_ALL_TANKS_SIGNED);
    end
end
...
function RaidGroups:processMigrations(Migrations, numberOfMigrations, index)
    ...
    elseif (action == "set") then
        ...
        if (not leftIndex) then
            return GL:warning(string.format(L.LANG_GROUP_SOMETHING_WENT_WRONG_MOVING, playerName));
        end
    ...
    if (index < numberOfMigrations) then
        ...
        GL.Ace:ScheduleTimer(function ()
            ...
            GL:success(L.LANG_FINENISHED_RAID_ROSTER);
        end, 2);
    end
end
