...
function PlusOnes:handleWhisperCommand(_, message, sender)
    ...
    if (args[2]) then
        ...
        GL:sendChatMessage(
            string.format(L.LANG_PLUS_PLAYER, GL:capitalize(name), plusOne),
            "WHISPER", nil, sender
        );
        return;
    end
    ...
    local plusOne = self:getPlusOnes(name);
    GL:sendChatMessage(
        string.format(L.LANG_PLUS_YOURE, plusOne),
        "WHISPER", nil, sender
    );
end
...
function PlusOnes:import(data, openOverview, MetaData)
    ...
    if (type(data) ~= "string"
        ...
        GL.Interface:get("PlusOnes.Importer", "Label.StatusMessage"):SetText(L.LANG_PLUS_INVALID);
        return false;
    end
    ...
    if (GL:empty(Totals)) then
        local errorMessage = L.LANG_PLUS_INVALID_DATA;
...
function PlusOnes:broadcast()
    ...
    local Broadcast = function ()
        GL:message(L.LANG_PLUS_BROADCASTING_DATA);
        ...
        GL.CommMessage.new(
            ...
            Label = GL.Interface:get(GL.PlusOnes, "Label.BroadcastProgress");
            if (Label) then
                Label:SetText(string.format(L.LANG_BROADCAST_SENT, sent, plusOne));
            end
        end);
    end
...
function PlusOnes:receiveBroadcast(CommMessage)
    ...
    local importBroadcast = (function ()
        ...
        GL:warning(L.LANG_PLUS_ATTEMP .. CommMessage.Sender.name);
    ...
    elseif (not GL:empty(uuid)) then -- This is a different dataset, not an update
        question = string.format(
            L.LANG_PLUS_CLEAR_EXISTING_DATA,
            CommMessage.Sender.name
        );
    else
        question = string.format(
            L.LANG_PLUS_IMPORT_NEW_DATA,
            CommMessage.Sender.name
        );
    end
...
function PlusOnes:broadcastUpdate(playerName, plusOne, delete)
    ...
    GL:message(L.LANG_PLUS_BROADCAST_IN_PROGRESS);
