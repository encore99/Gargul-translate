...
function TMB:tooltipLines(itemLink)
    ...
    if (GL.Settings:get("TMB.showItemInfoOnTooltips")) then
        ...
        if (not GL:empty(TMBTier)
            ...
            local tierString = string.format(L.LANG_TMB_R,
    ...
    if (itemIsOnSomeonesPriolist
        ...
        tinsert(Lines, string.format("\n|cFFff7a0a%s|r", source .. L.LANG_TMB_PRIOLIST));
    ...
    if (itemIsOnSomeonesWishlist
        ...
        -- Add the header
        tinsert(Lines, string.format("\n|cFFffffff%s|r", L.LANG_TMB_WISHLIST));
...
function TMB:import(data, triedToDecompress, source)
    ...
    local function displayGenericException()
        GL.Interface:get("TMB.Importer", "Label.StatusMessage"):SetText(L.LANG_TMB_INVALID_DATA);
    end
    ...
    if (firstLine:match("^[0-9]+,")) then
        ...
        if (not data) then
            return GL.Interface:get("TMB.Importer", "Label.StatusMessage"):SetText(L.LANG_INVALID_CSV);
        end
    ...
    elseif (GL:strStartsWith(firstLine, '"')) then
        ...
        if (not data) then
            return GL.Interface:get("TMB.Importer", "Label.StatusMessage"):SetText(L.LANG_INVALID_TMB_OR_DFT_DATA);
        end
    end
    ...
    if (type(data) ~= "table") then
        ...
        if (not jsonDecodeSucceeded
            ...
            GL.Interface:get("TMB.Importer", "Label.StatusMessage"):SetText(L.LANG_INVALID_DFT_DATA);
            return false;
        end
    end
    ...
    if (not GL:empty(PlayersWithoutTMBDetails)) then
        ...
        GL:warning(string.format(L.LANG_TMB_NO_ENTRIES, self:source()));
...
function TMB:decompress(data)
    ...
    if (not base64DecodeSucceeded) then
        local errorMessage = L.LANG_BASE_DECODING;
    ...
    if (not zlibDecodeSucceeded) then
        local errorMessage = L.LANG_ZLIB_DECODING;
        GL.Interface:get("TMB.Importer", "Label.StatusMessage"):SetText(errorMessage);
        return "";
    end
...
function TMB:broadcast(sendEmptyPayload)
    ...
    if (self.broadcastInProgress) then
        GL:error(L.LANG_BROADCAST_PROGRESS);
        return false;
    end
    ...
    if (GL.User.isInGroup
        ...
        GL:warning(L.LANG_PERMISSION_NEED);
        return false;
    end
    ...
    if (not self:available()) then
        GL:warning(L.LANG_NOTHING_TO_BROADCAST);
        return false;
    end
...
function TMB:broadcastToWhitelist()
    ...
    if (numberOfPlayers < 1) then
        GL:warning(L.LANG_NO_ONE_IN_GROUP_BROADCAST);
        self.broadcastInProgress = false;
        return;
    end
    ...
    if (UnitAffectingCombat("player")) then
        GL:message(L.LANG_BROADCAST_COMBAT_DELEY);
...
function TMB:broadcastToGroup()
    ...
    local broadcast = function ()
        ...
        GL:message(L.LANG_BROADCASTING_TMB_DATA);

        local Label = GL.Interface:get(GL.TMB, "Label.BroadcastProgress");

        if (Label) then
            Label:SetText(L.LANG_BROADCASTING);
        end
        ...
        GL.CommMessage.new(
            ...
            GL:success(L.LANG_BROADCASTING_DONE_TMB);
            Events:fire("GL.TMB_BROADCAST_ENDED");
            self.broadcastInProgress = false;

            Label = GL.Interface:get(GL.TMB, "Label.BroadcastProgress");
            if (Label) then
                Label:SetText(L.LANG_BROADCASTING_DONE);
            end
            ...
            Label = GL.Interface:get(GL.TMB, "Label.BroadcastProgress");
            if (Label) then
                Label:SetText(string.format(L.LANG_BROADCAST_SENT, sent, total));
            end
        end);
    end
...
function TMB:receiveBroadcast(CommMessage)
    ...
    if (not GL:empty(Data)) then
        GL:warning(L.LANG_BROADCAST_PROGRESS_INCOMING_DATA_FROM .. CommMessage.Sender.name);
        ...
        if (type(Data) ~= "table" or GL:empty(Data)
            ...
            GL:error(L.LANG_TMB_INVALID_TMB_DATA_RECEIVED_FROM .. CommMessage.Sender.name);
            return;
        end
